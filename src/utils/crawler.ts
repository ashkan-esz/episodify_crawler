...
export function replacePersianNumbers(input: string): string {
    ...
    for (let i = 0; i < 10; i++) {
        input = input.replace(persianNumbers[i], i.toString())
...
export function getYear(title: string, page_link: string,
     downloadLinks: DownloadLink[]): string | undefined {
...
export function getSeasonEpisode(input: string, isLinkInput: boolean = false) {
    try {
        ...
        if (case0) {
            ...
            season = Number(temp?.[0].replace('_', ''));
        ...
        if (season === 0 || episode === 0) {
            ...
            if (episodeMatch) {
                ...
                .find(item => item.includes('e')) || episodeMatch.pop();
...
export function checkBetterQuality(quality: string, prevQuality: string,
     withSubIsBetter: boolean = true, handleMalformedInfo: boolean = false): boolean {
...
export function removeDuplicateLinks(input: DownloadLink[], replaceInfo: boolean = false, replaceBadInfoOnly: boolean = false): DownloadLink[] {
...
export function getMonthNumberByMonthName(monthName: string): string {
    ...
    return months[
        monthName.slice(0, 3).toLowerCase() as keyof typeof months];
}
